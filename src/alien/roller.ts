...
export class V5Roller extends Roller<Dice, Faces, DicePool> {
    ...
    public roll(rolls: DicePool): Array<Roll<Dice, Faces>> {
        return [
            ...rollDie(rolls.stress, Dice.STRESS, STRESS_ROLL_TABLE, this.rng),
        ...
        return Mustache.render(
            base,
            {
                ...
                rolls: rolls.map((roll) => {
                    const isstressDie = roll.die === Dice.STRESS;
                    return new DieRollView(roll, dieRollImages, isstressDie);
                }),
    ...
    protected toDicePool(dice: Dice[]): DicePool {
        const stress = countMatches(dice, (die) => die === Dice.STRESS);
        const skills = countMatches(dice, (die) => die === Dice.SKILL);
        return new DicePool(skills, stress);
    }
